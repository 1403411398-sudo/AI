...
class Embeddings:
    ...
    def interpolate_pos_encoding(self, embeddings, height, width):
        # ************* ToDo, resize the self.position_embeddings *************
        
        # shape: (1, N_patches, D)
        np_pos_embed = self.position_embeddings[:, 1:, :] 
        
        N = np_pos_embed.shape[1]
        grid_old = int(np.sqrt(N))
        
        h_new = height // self.patch_size
        w_new = width // self.patch_size
        
        if grid_old == h_new and grid_old == w_new:
            return self.position_embeddings

        np_pos_embed = np_pos_embed.reshape(1, grid_old, grid_old, self.hidden_size)
        
        zoom_factors = (1, h_new / grid_old, w_new / grid_old, 1)
        
        np_pos_embed = zoom(np_pos_embed, zoom_factors, order=1)
        
        np_pos_embed = np_pos_embed.reshape(1, -1, self.hidden_size)
        return np.concatenate([self.position_embeddings[:, :1, :], np_pos_embed], axis=1)

    def __call__(self, pixel_values):
        B, _, H, W = pixel_values.shape

        patch_values = self.pixel2patches(pixel_values) # (B, h*w, C*ps**2)
        ...
        pos_embed  = self.interpolate_pos_encoding(embeddings, H, W) # (B, N+1, D)
...
class MultiHeadAttention:
    ...
    def __call__(self, x):
        ...
        attn_score = (q @ k.transpose(0, 1, 3, 2)) / np.sqrt(self.head_dim)
        attn_probs = softmax(attn_score, axis=-1)
